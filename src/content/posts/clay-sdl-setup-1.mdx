...
Sadly, my current experience writing desktop applications lies in stark contrast with the lofty goal of a feature-rich desktop application. In fact, it occurred to me that, after nearly three years spent mostly on web dev, the only way I know to draw a rectangle on the screen looks something like this:
...
First, let's create a file `common.h` and define a preliminary `AppState` type:
...
We're going to define `SDL_AppInit` in `init.c`, and an associated header file `init.h` is as good a place as any to define the `SDL_MAIN_USE_CALLBACKS` macro:
...
Accordingly, we will check operations that can fail, such as allocating memory for the `AppState` struct, and then decide how to handle their failure. Some errors should result in the application immediately terminating. Failure to initialise app state or create a window are things that require immediate termination. Other errors require handling gracefully, such as by writing them to an error log, or informing the user something they tried to do didn't succeed.
...
We should get in a habit of destroying the things we create on app teardown as soon as possible, so let's create `quit.h` and `quit.c` next:
...
`event.h` and `event.c` will be fairly sparse for now. We're just going to listen for the escape key, and signal to terminate the app succesfully if it's encountered:
