...
We're going to define `SDL_AppInit` in `init.c`, and an associated header file `init.h` is as good a place as any to define the `SDL_MAIN_USE_CALLBACKS` macro and include the `SDL_main.h` header:
